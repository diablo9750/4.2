...
namespace types
{
	...
	void game::Out(ofstream& ofst) {
		...
		ofst << "Количество гласных в названии: " << Vowel() << endl;
	}
	...
	void doc::InData(ifstream& ifst) {
		ifst >> country >> name >> date;
	}

	//Вывод параметров мультфильма в поток
	void doc::Out(ofstream& ofst) {
		ofst << "Это доументальный фильм.Страна, где произведён фильм: " << country << ", Название фильма: " << name << ", Год выпуска: " << date << endl;
		ofst << "Количество гласных в названии: " << Vowel() << endl;
	}
